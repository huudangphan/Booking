using Modetour.B2C.Entity.Package.Json;
...
namespace Modetour.B2C.Commons.Helper
{
    public class APIHelper
    {
        ...
        public static NationModel ConvertJsonToObjectNationModel(string json)
        {
            try
            {
                var result = Newtonsoft.Json.JsonConvert.DeserializeObject<NationModel>(Functions.ToString(json));
                return result;
            }
            catch (Exception ex)
            {
        ...
        public static FilterModel ConvertJsonToObjectFilterModel(string json)
        {
            try
            {
                var result = Newtonsoft.Json.JsonConvert.DeserializeObject<FilterModel>(Functions.ToString(json));
                return result;
            }
            catch (Exception ex)
            {
        ...
        public static string SumParam(object param)
        {
            ...
            {
                object val = property.GetValue(param);

                IEnumerable<object> enumerable = val as IEnumerable<object>;
                if (val != null && val.GetType().IsArray)
                {
                    foreach (object item in enumerable)
                    {
                        //result.Append(string.Format("&{0}={1}", property.Name, item));
                        result.Append(string.Format("&{0}={1}", property.Name, HttpUtility.UrlEncode(item.ToString())));
                    }
                }
                else
                {
                    //result.Append(string.Format("&{0}={1}", property.Name, val));
                    result.Append(string.Format("&{0}={1}", property.Name, val == null ? string.Empty : HttpUtility.UrlEncode(val.ToString())));
                }
            }
